...
**Stores are the shared state.** In-memory, `sync.RWMutex`-guarded caches passed by pointer so multiple services see the same data. All stores are keyed by a *server key* — `models.ServerKey(oba_base_url, agency_id)`, the sanitized base URL and the agency joined by `|`. Agency IDs are only unique within one OBA deployment, so nothing may be keyed on `agency_id` alone:
- `gtfs.StaticStore` — parsed GTFS static bundles. In agency-mode it stores only the configured agency's owned snapshot (routes, referenced services, stops, and parent stations). In server-mode one merged bundle pointer is registered under every agency `agency.txt` declares; its stop slice is the server-wide flattened set.
...
- `geo.BoundingBoxStore` — geographic bounds per server key (used to flag out-of-bounds vehicles). Agency-mode bounds come only from the retained snapshot and do not use the server union; server-mode retains the existing per-agency and union boxes.
- `gtfs.RouteAgencyIndex` — `route_id` and `trip_id` → `agency_id` (plus `agency_id` → `agency_name`), built during static-bundle parse and keyed by composite `ServerKey`. It is shared by agency-mode RT filtering and server-mode vehicle attribution.
...
- **The server-mode vehicle pass runs once per server per tick, never once per agency.** `collectVehicleMetrics` walks the consolidated GTFS-RT feed and attributes each vehicle to its owning agency through `gtfs.RouteAgencyIndex` (`route_id` and `trip_id`). Calling it per agency re-walks the same merged feed, which permanently inflates the `VehicleReportCount` counter and files every vehicle under every agency's last-seen slot. In agency-mode, the realtime store is already filtered and the pass consumes that configured entry's snapshot. Its `agencies` argument is the scope dispatch: `nil` means agency-mode, non-nil means server-mode. This is why the per-agency probes live in `collectAgencyChecks` (backoff, ping, bundle expiration, OBA API metrics, vehicles-for-agency) — both modes call them once per agency, while the server-mode vehicle pass is deliberately outside that loop.
