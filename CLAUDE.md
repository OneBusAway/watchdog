...
- `geo.BoundingBoxStore` — geographic bounds per server key (used to flag out-of-bounds vehicles). Each agency gets its own bounding box computed from its stops; the server-scoped key (empty agency) stores the union box as a fallback for unattributed vehicles.
