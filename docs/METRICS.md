...
- **Bounding box accuracy depends on attribution:** `gtfs_rt_stopped_out_of_bounds_vehicles` is attributed per agency. Attributed vehicles are validated against their own agency's bounding box. Unattributed vehicles fall back to the server-wide union box — on a multi-agency server that rectangle covers all agencies' stops, so the empty-`agency_id` series is a looser bound than the per-agency ones. Agency-mode entries are validated against the configured agency's own bounding box, never the union.
