...
| `gtfs_rt_last_successful_fetch_timestamp_seconds` | Gauge | `server_name`, `server_url` | unix_timestamp | When Watchdog last successfully fetched and parsed all configured vehicle-position feeds for the server. Use this to gate current-state feed metrics; Prometheus sample timestamps only prove Watchdog was scraped. |
...
| `oba_vehicles_last_successful_fetch_timestamp_seconds` | Gauge | `agency_id`, `agency_name`, `server_name`, `server_url` | unix_timestamp | When Watchdog last received a successful vehicles-for-agency response. |
