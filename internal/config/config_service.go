...
func (cs *ConfigService) RefreshConfig(ctx context.Context, url, authUser, authPass string, interval time.Duration, maxRetries int, onUpdated func([]models.ObaServer)) {
	refreshConfig(ctx, cs.Client, url, authUser, authPass, cs.Config, cs.DroppedStore, cs.Logger, interval, maxRetries, onUpdated)
}
...
func LoadConfigFromFile(filePath string, logger *slog.Logger, droppedStore *DroppedServersStore) ([]models.ObaServer, error) {
	servers, err := loadConfigFromFile(filePath, logger, droppedStore)
	if err != nil {
...
func LoadConfigFromURL(ctx context.Context, client *http.Client, url, authUser, authPass string, maxRetires int, logger *slog.Logger, droppedStore *DroppedServersStore) ([]models.ObaServer, error) {
	servers, err := loadConfigFromURL(ctx, client, url, authUser, authPass, maxRetires, logger, droppedStore)
	if err != nil {
