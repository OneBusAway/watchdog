...
func checkBundleExpiration(staticStore *gtfs.StaticStore, currentTime time.Time, server models.ObaServer) (int, int, error) {
	...
	BundleEarliestExpirationGauge.WithLabelValues(server.AgencyID, server.AgencyName).Set(float64(daysUntilEarliestExpiration))
	BundleLatestExpirationGauge.WithLabelValues(server.AgencyID, server.AgencyName).Set(float64(daysUntilLatestExpiration))
